//! Minimal SHA-256 (FIPS 180-4), written in-crate so the single-binary,
//! no-extra-dependencies policy holds. Used for the flow fingerprint that
//! guards `--resume`: FNV-1a is not collision-resistant, so a crafted flow
//! could share a fingerprint with a different one and slip past the check.
...
mod tests {
    ...
    // FIPS 180-4 / NIST test vectors. The one-million-'a' case exercises the
    // multi-block path; the others cover empty, one-block and two-block input.
    #[test]
    #[allow(clippy::useless_vec)] // 1M bytes stay on the heap, not the stack
    fn known_vectors() {
        assert_eq!(
            hex(b""),
    ...
    fn padding_boundaries() {
        // 55 bytes: length fits the first block exactly after the 0x80 byte.
        // 56 bytes: forces one extra block. 64 bytes: a full block alone.
        assert_eq!(
            hex(&[b'x'; 55]),
